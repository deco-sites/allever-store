    ...
    return (<div class="">
      ...
      <div class="flex flex-col  gap-3">
        <div class="flex flex-row gap-4">
          {certifieds?.slice(0, 2).map(({ image, alt, link, width, height }, index) => (<div key={index} class="rounded flex justify-center items-center cursor-pointer">
              <a href={link} target="_blank">
                <Image src={image} alt={alt || "Certified image"} width={width || 53} height={height || 34} loading="lazy"/>
              </a>
            </div>))}
        </div>
        <div class="flex flex-row gap-4">
          {certifieds?.slice(2, 4).map(({ image, alt, link, width, height }, index) => (<div key={index} class="rounded flex justify-center items-center cursor-pointer">
              <a href={link} target="_blank">
                <Image src={image} alt={alt || "Certified image"} width={width || 53} height={height || 34} loading="lazy"/>
              </a>
            </div>))}
...
function Service({ title, label, button }: Service) {
    return (<div class="flex gap-[21px] flex-col max-w-[200px]">
      {title && <p class="text-base font-normal">{title}</p>}
      {label && <p class="text-base font-normal text-[#888888]">{label}</p>}
      {button?.label && button?.link && (<a href={button.link} target="_blank" class="flex justify-center gap-[10px] px-5 py-[13px] bg-[#123ADD] text-white rounded-[20px] hover:bg-blue-600 text-center">
...
function Payments({ title, paymentMethods }: PaymentsProps) {
    return (<>
      ...
      <ul class="flex flex-wrap gap-2">
        {paymentMethods?.map(({ image, alt }, index) => (<li key={index} class="border border-base-100 rounded flex justify-center items-center">
            <Image src={image} alt={alt || "Payment method image"} width={53} height={34} loading="lazy"/>
          </li>))}
      </ul>
    </>);
}
function Footer({ links = [], social = [], logo, paymentMethods, copyright, service, certified, label = "Siga a [allever] nas redes sociais!", }: Props) {
    const device = useDevice();
    return (<>
      {device === "desktop" &&
            (<footer class="px-5 sm:px-0 pt-10" style={{ backgroundColor: "#FFF" }}>
              ...
              <div class="flex space-between">
                <div class="flex flex-col">
                  ...
                  <ul class="flex mt-5 gap-[3px]">
                    {social.map(({ image, href, alt }) => (<li class="bg-[#123ADD] p-[9px] rounded-[7px]">
                        <a href={href} target="_blank">
                          <Image src={image} alt={alt} loading="lazy" width={19} height={19}/>
                        </a>
                      </li>))}
                  </ul>
                </div>
                <ul class="flex w-[80%] space-between">
                  <li class="flex flex-col gap-4">
                    {links[0] && (<>
                        <a class="text-base font-normal" target="_blank" href={links[0].href}>
                          {links[0].title}
                        </a>
                        <ul class="flex flex-col  gap-[21px] mb-[30px]">
                          {links[0].children &&
                        links[0].children.map(({ title, href }) => (<li>
                                <a class="text-base font-normal text-[#888888]" href={href}>
                                  {title}
                                </a>
                              </li>))}
                        </ul>
                    ...
                    {links[1] && (<>
                        <a class="text-base font-normal" target="_blank" href={links[3].href}>
                          {links[3].title}
                        </a>
                        <ul class="flex flex-col gap-[21px]">
                          {links[3].children &&
                        links[3].children.map(({ title, href }) => (<li>
                                <a class="text-base font-normal text-[#888888]" href={href}>
                                  {title}
                                </a>
                              </li>))}
                  ...
                  <li class="flex flex-col gap-4">
                    {links[2] && (<div class="flex gap-4 mb-[30px] flex-col">
                        <a class="text-base font-normal" target="_blank" href={links[1].href}>
                          {links[1].title}
                        </a>
                        <ul class="flex flex-col  gap-[21px]">
                          {links[1].children &&
                        links[1].children.map(({ title, href }) => (<li>
                                <a class="text-base font-normal text-[#888888]" href={href}>
                                  {title}
                                </a>
                              </li>))}
                    ...
                    {links[3] && (<Payments title="Formas de Pagamento" paymentMethods={paymentMethods?.paymentMethods}/>)}
                  </li>

                  <li class="flex flex-col gap-4">
                    {links[3] && (<>
                        <div class="flex flex-col gap-[21px] pb-[30px]">
                          <a class="text-base font-normal" target="_blank" href={links[2].href}>
                            {links[2].title}
                          </a>
                          <ul class="flex flex-col gap-[21px]">
                            {links[2].children &&
                        links[2].children.map(({ title, href }) => (<li>
                                  <a class="text-base font-normal text-[#888888]" href={href}>
                                    {title}
                                  </a>
                                </li>))}
                  ...
                  <li class="flex flex-col gap-4">
                    <>
                      <Service title={service?.title} label={service?.label} button={service?.button}/>
                    </>
                  </li>
                </ul>
              </div>
      ...
      {device === "mobile" &&
            ...
            <div class="flex flex-col py-[35px] mt-5">
              ...
              <ul class="flex mt-5 gap-[10px]">
                {social.map(({ image, href, alt }) => (<li class="bg-[#123ADD] p-[9px] rounded-[7px]">
                    <a href={href} target="_blank">
                      <Image src={image} alt={alt} loading="lazy" width={25} height={25}/>
                    </a>
                  </li>))}
              </ul>
            </div>
            <div>
              {links.map(({ title, children }, index) => (<ul key={index} class="flex flex-col gap-2 mb-[10px]">
                  <Collapsable class="bg-white py-[29px] rounded-[20px]" title={<div class=" px-5 flex flex-row space-between  items-center">
                    ...
                    {children &&
                        children.map(({ title, href }) => (<li class="flex flex-row gap-[10px] px-5 mt-5" key={href}>
                          <a class="text-base font-normal text-[#888888]" target="_blank" href={href}>
                            {title}
                          </a>
                        </li>))}
                  </Collapsable>
