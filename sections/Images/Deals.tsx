        ...
        <div className="py-[45px] lg:container">
            {device === "mobile" ? (
                <div className="overflow-x-auto no-scrollbar px-5">
                    <div className="flex space-x-4 animate-scroll">
                        {deals.map((deal, index) => (
                            <div key={index} className="flex items-start gap-[15px] min-w-[213px]">
                                <Image
                                ...
                                <div className="flex items-start flex-col">
                                    <p className="text-base font-semibold text-[#000]">{deal.title}</p>
                                    <p className="text-base font-normal text-[#a8a8a8]">{deal.label}</p>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="flex space-between gap-4">
                    {deals.map((deal, index) => (
                        <div key={index} className="flex items-start gap-[15px] lg:max-w-[214px]">
                            <Image
                                src={deal.image ? deal.image : ''}
                                alt={deal.title || 'Imagem de condição'}
                                width={deal.width || width}
