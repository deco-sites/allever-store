...
const CampaignTimer = ({ expiresAt, onExpire, hideLabel }: CampaignTimerProps) => {
    ...
    return (
        <div class="flex gap-5 justify-center text-white text-center items-center my-4 lg:mb-0">
            <div>
                <p class="text-xl lg:text-4xl font-bold flex gap-3">{formatTime(timeLeft.Dias)} <span>:</span></p>
                {hideLabel ? <p class="text-xs lg:text-sm">Dias</p> : null}
            </div>
            <div>
                <p class="text-xl lg:text-4xl font-bold flex gap-3">{formatTime(timeLeft.Horas)}  <span>:</span></p>
                {hideLabel ? <p class="text-xs lg:text-sm">Horas</p> : null}
            </div>
            <div>
                <p class="text-xl lg:text-4xl font-bold flex gap-3">{formatTime(timeLeft.Minutos)} <span>:</span></p>
                {hideLabel ? <p class="text-xs lg:text-sm">Min</p> : null}
            </div>
            <div>
                <p class="text-xl lg:text-4xl font-bold flex gap-3">{formatTime(timeLeft.Segundos)}</p>
