      ...
      <>
        ...
        <div class="col-start-1 col-span-3 row-start-1 row-span-1 leading-[1]">
          <Slider class="carousel carousel-center sm:carousel-end gap-[0.5rem] lg:gap-5 w-full ">
            {products?.map((product, index) => (<Slider.Item index={index} class={clx("carousel-item", "first:pl-5 first:sm:pl-0", "last:pr-5 last:sm:pr-0")}>
                <ProductCard flags={flags} index={index} product={product} itemListName={itemListName} class="w-[287px] sm:w-[300px]"/>
              </Slider.Item>))}
          </Slider>
        </div>
