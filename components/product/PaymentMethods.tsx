...
function PaymentMethods({ offers, installment }: PaymentMethodsProps) {
    const offer = offers?.offers.find((o) => o.availability === "https://schema.org/InStock") || offers?.offers[0];
    const maxIntallments = offer?.priceSpecification.reduce((acc: UnitPriceSpecification | null, curr: UnitPriceSpecification) => {
    ...
    hx-on:click={useScript(() => document.getElementById('payment-methods')?.showModal())} class="underline text-signature-blue text-left">
        ...
        <div class="modal-box bg-white">
          ...
          <div className="flex">
            <div class="flex flex-col gap-8">
              <button id="pix" class="flex flex-col items-center gap-2 text-sm font-semibold text-center text-black max-w-24" hx-on:click={useScript(() => {
            ...
            installmentsButton?.classList.remove('border-signature-blue');
              ...
              <button id="installments" class="flex flex-col items-center gap-2 text-sm font-semibold text-center text-black max-w-24" hx-on:click={useScript(() => {
            const content = document.querySelector('div#pix');
            const otherContent = document.querySelector('div#installments');
            ...
            if (maxIntallments?.name === priceSpecification.name) {
                const { billingDuration, billingIncrement, } = priceSpecification;
                return (<tr key={index} class="odd:bg-light-gray even:bg-transparent">
