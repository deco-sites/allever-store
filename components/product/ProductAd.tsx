...
export default function ProductAd({
    ...
    return (
        <div class="container p-3 lg:py-0 sm:px-0 flex flex-col lg:flex-row">
            <div class={` gap-3 p-3 rounded-xl my-5 min-h-[320px] max-h-[320px] flex items-center mx-auto`}>
                <a
                    href={url}
            ...
            <div class={`flex flex-col items-start justify-center p-3`}>
                <p class="mb-[5px] text-[#D3D3D3] text-sm leading-[21px] uppercase">{brand?.name}</p>
                <p class="text-base leading-6 font-normal text-black max-w-[252px]">{name}</p>
                <div class="flex items-start lg:items-center mt-[5px] flex-col lg:flex-row lg:gap-[6px]">
                    {offers?.lowPrice === price ? <p class="text-sm leading-[21px] text-[#989898] line-through"> {formatPrice(offers?.lowPrice)} </p> : null }
                    <p class="font-semibold sm:text-right text-xl lg:text-[20px] text-[#0066E4]">{formatPrice(price)}</p>
                </div>
            </div>
        </div>
    );
