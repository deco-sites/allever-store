...
export default function Searchbar({ placeholder = "O que deseja?", loader, searchBarDrawer }: SearchbarProps) {
    const slot = useId();
    return (<div class={`search-bar-wrapper w-full relative lg:hover:w-[50vw] ${searchBarDrawer ? "gap-[unset]" : ""}`}>
      <div class="search-bar-container flex items-center justify-end gap-2">
        <button type="submit" form={SEARCHBAR_INPUT_FORM_ID} class="bg-transparent border-none hidden md:block w-6 p-0" aria-label="Search" tabIndex={-1}>
        ...
        <form id={SEARCHBAR_INPUT_FORM_ID} action={ACTION} class={`join flex gap-[20px] ${searchBarDrawer ? "my-5 w-full px-5" : ""}`}>
          <button type="submit" form={SEARCHBAR_INPUT_FORM_ID} class="bg-transparent border-none md:hidden" aria-label="Search" tabIndex={-1}>
            {searchBarDrawer
            ? (<Icon id="search-drawer" class="inline"/>)
            : <Icon id="search" class="inline"/>}
          </button>
          <input type="text" name={NAME} class={`rounded-[30px] outline-none py-[8.5px] px-5 placeholder-[#D3D3D3] w-full lg:w-auto`} placeholder={placeholder} autocomplete="off" autoComplete="off" data-hx-target={`#${slot}`} data-hx-post={loader &&
