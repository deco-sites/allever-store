...
function WishlistButton({ item, variant = "full", pdp }: Props) {
    ...
    return (<>
      <button id={id} data-wishlist-button disabled {...addToWishlistEvent} aria-label="Add to wishlist" hx-on:click={useScript(onClick, productID, productGroupID)}>
        {pdp
            ? (<Icon id="favorite-pdp" class="[.htmx-request_&]:hidden" fill="none"/>) : (<Icon id="wishlist-icon" class="[.htmx-request_&]:hidden" fill="none"/>)}
