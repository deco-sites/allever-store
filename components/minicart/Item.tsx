...
function CartItem({ item, index, locale, currency }: Props) {
    ...
    return (<fieldset 
    // deno-lint-ignore no-explicit-any
    data-item-id={(item as any).item_id} class="grid grid-rows-1 gap-2 bg-white py-2 px-[10px] rounded-[10px]" style={{ gridTemplateColumns: "auto 1fr" }}>
      <Image alt={name} src={image} width={140} height={192} class="h-full object-contain"/>
